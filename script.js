...
let isMonthly = false;
let chart1, chart2, chartDoughnut;
...
document.addEventListener('DOMContentLoaded', () => {
    fetch('data.json')
        .then(res => res.json())
        .then(data => {
            const excludeVessels = ['abu samrah', 'super fast beleares', 'superfast baleares'];
            rawData = data.filter(d => !excludeVessels.includes(d.vessel.toLowerCase()));
...
function getProcessedData() {
    if (!isMonthly) return rawData;
    return rawData.map(d => ({ ...d, week: 'Month 1' }));
}
...
function renderSummaryCards() {
    ...
    const dataToUse = getProcessedData();
    
    const weekStats = weeks.map(w => {
        const data = dataToUse.filter(d => d.week === w);
    ...
    weekStats.forEach(stat => {
        ...
        card.innerHTML = `
            <div class="summary-week">${weekLabels[stat.week] || stat.week}</div>
...
function renderCharts() {
    ...
    const totalFailures = weeks.map(w => dataToUse.filter(d => d.week === w).length);
    const uniqueShips = weeks.map(w => {
        const ships = dataToUse.filter(d => d.week === w).map(d => d.vessel);
        ...
        options: {
            ...
            scales: {
                y: { 
                    ...
                    grid: { color: '#2D3748' }, 
                    ticks: { color: '#A0AEC0' },
                    title: { display: true, text: 'No. of failures', padding: { bottom: 10 }, color: '#A0AEC0' }
                },
                y1: { 
                    beginAtZero: true, 
                    position: 'right', 
                    ...
                    ticks: { color: '#A0AEC0' },
                    title: { display: true, text: 'No. of ships', padding: { bottom: 10 }, color: '#A0AEC0' }
                },
                x: { 
                    grid: { display: false }, 
                    ticks: { color: '#A0AEC0' },
                    title: { display: true, text: 'Weeks', padding: { top: 10 }, color: '#A0AEC0' }
                }
            },
            plugins: {
    ...
    document.getElementById('aggregateSubtitle').innerText = `All ${isMonthly ? '1 month' : '4 weeks'} combined (${totalRecords} records)`;
...
function renderMatrix() {
    ...
    theadRow.innerHTML = '<th></th>' + weeks.map(w => `<th>${weekLabels[w] || w}</th>`).join('') + '<th>Total</th>';
...
function setupEventListeners() {
    ...
    document.getElementById('viewToggle').addEventListener('change', (e) => {
        ...
        document.getElementById('weeklyLabel').classList.toggle('active', !isMonthly);
        document.getElementById('monthlyLabel').classList.toggle('active', isMonthly);
        renderSummaryCards();
        renderCharts();
        renderMatrix();
    });
